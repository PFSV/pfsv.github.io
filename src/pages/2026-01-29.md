...
- 백엔드랑 붙는거 해야함(내일 내일모레 개발된다함)  -> 고정값으로 하책임이 테스트 할 수 있게 해준다고 함
    ...
    ** 13:14 
    json으로 프롬프트 정리 후, config.py에 함수 형태로, 스타일 관련 선택지를 넣으면 관련 프롬프트 룩업테이블에서 확인해 report_generate에 문자열 변수로 제공할 예정.
    스타일 관련 선택지는 pydantic request에 들어가야하고, session 내에 고정되나, 첫 generation 이후 필요할지는 의문 ... (모두 안고 추가 revision or 그냥 revision만)

- response는 answer: str, SSE 방식으로 해서 classify, generate, output 단계에서 상태값 전달 (langraph SSE 변환 -> requery 코드 참조) // 동시에 중간중간 로그도 찍히게 바꿀 것
...
* 사규내규 검색시스템(Airail)
 ...
 => 먼저 해야할 것
    DRM(문서 보안)이 걸려있어서, 풀고csv로드
    ** 1.분서필요 -> 정상 질문
    ** 2.의미없음외 -> 비정상 질문(false negative 일부 포함)
